...
def make_png(directory, label, information, moments, bin_centers):
...
def make_plots(exposure, core_directory, psf_type, number_of_angular_bins):
    ...
    for label in ["test", "train"]:
        ...
        information = np.empty([3*len(moments),number_of_angular_bins])
        ...
        for m, moment in enumerate(moments):
            for k, kind in enumerate(kinds):
                ...
                kind_final_moment, bin_edges, binnumber = stats.binned_statistic(x=angles,values=kind_moment, statistic="mean",bins=number_of_angular_bins, range=(0.0,360.0)) #Here the average moment values inside polar angular bins are computed.
                if m == 0 and k == 0:
        ...
        make_png(directory=directory, label=label, information=information, moments=moments, bin_centers=bin_centers)
...
if __name__ == '__main__':
    ...
    make_plots(exposure=exposure, core_directory=core_directory, psf_type=psf_type, number_of_angular_bins=number_of_angular_bins)
