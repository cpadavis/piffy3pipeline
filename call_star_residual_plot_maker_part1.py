...
def make_call(psf_type):
    ...
    for exposure_i, exposure in enumerate(exposures):
        try:
            ...
            terminal_command = "bsub -W 300 -o {0}/log_file_dump/{1}_call_star_residual_plot_maker_{2}.txt python {0}/star_residual_plot_maker.py --core_directory {0} --psf_type {1} --exposure {2}".format(core_directory, psf_type, exposure)
